...
class CountableMapTest extends TestCase
{
    ...
    /**
     * Tests that the constructor is working as expected when given a traversable.
     *
     * @since [*next-version*]
     */
    public function testConstructorTraversable()
    {
        $elements = [uniqid('key') => uniqid('val')];
        $traversable = $this->createTraversable($elements);
        $subject = $this->createInstance(['_setMany'], [], true);
        $_subject = $this->reflect($subject);
        ...
        $subject->__construct($traversable);
    }
